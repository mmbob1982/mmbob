  ...
  <div id="content" class="clearfix">
    ...
    <div id="rightColumn">
      <ul class="list_headers">
		<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=46be8e963463ee3edf7e3095d9373778&sql=select+%2A+from+v9_yp_company+%24con+order+by+userid+desc&num=81&cache=3600&return=data\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$tag_cache_name = md5(implode('&',array('sql'=>'select * from v9_yp_company $con order by userid desc',)).'46be8e963463ee3edf7e3095d9373778');if(!$data = tpl_cache($tag_cache_name,3600)){pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("select * from v9_yp_company $con order by userid desc LIMIT 81");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);if(!empty($data)){setcache($tag_cache_name, $data, 'tpl_data');}}?>
	    <?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
		<?php $n=1; if(is_array($data)) foreach($data AS $key => $v) { ?>
        <li>
